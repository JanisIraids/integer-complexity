...
public:
  ...
  {
    std::cout << "Test the hypothesis, that f(2^a * 3^b * 5^c) = 2a + 3b + 5c for all a, b, c, with c < 6" << std::endl;
  }
  ...
  virtual void _perform(const std::vector<std::string> args)
  {
	...
	for (ull k = 0, k5 = 1; i2*j3*k5 < ret->size && k < 6; k++, k5 *= 5)
	  if (ret->f(i2*j3*k5) < 2*i+3*j+5*k)
	    out << "f(2^" << i << "*3^" << j << "*5^" << k << ") = "<< ret->f(i2*j3*k5) << " < " << 2*i + 3*j + 5*k << std::endl;
    if (PRINT)
...
Collapse(std::ostream& outs) : ExperimentWRet(outs) {};
  ...
  virtual void printUsage()
  {
    std::cout << "2 parameters: file from which to read good numbers and file that contains numbers to collapse" << std::endl;
    std::cout << "Prints the power in which a number collapses by repeatedly attempting division with good numbers or subtracting 1" << std::endl;
  }
  ...
  virtual void _perform(const std::vector<std::string> args)
  {
	...
	out << coll[i] << " " << icompl << " " << collapsesAt << " " << powcompl << " < " << pow*icompl << std::endl;
      mpz_clear(m_prime);
      mpz_clear(m_pow);
      mpz_clear(m_minus1);
    }
...
protected:
  ...
  virtual ui complexity(mpz_t m_n, int depth)
  {
	...
	{
	  if (totalFactors - curri >= FACTORS + 1 && !(mpz_cmp(factors[depth][j-1], factors[depth][j]) == 0 && currfactori[curri] != j-1))
	  {
	...
	if (!found)
	{ // remove last factor and try to find another with greater index
...
Collapse2(std::ostream& outs) : Collapse<PRINT>(outs) {};
  ...
  {
    std::cout << "2 parameters: file from which to read good numbers and file that contains numbers to collapse" << std::endl;
    std::cout << "Prints the power in which a number collapses by repeatedly attempting division with good numbers and then factorizing and doing a full search by trying each subdivision of factors into factor groups where each group is subtracted 1" << std::endl;
  }
...
public:
  ...
  virtual void printUsage()
  {
    std::cout << "Print the histogram of average digit in k-nary and logarithmic complexity up to l in a m x n bw image printed to file x" << std::endl;
  }
...
protected:
  virtual void _perform(const std::vector<std::string> args)
  {
    ...
    for (int row = 0; row < imgdimy; row++)
      ...
      {
	im[row][col] = png::gray_pixel(max(min(static_cast<int>(floor(static_cast<double>(bins[imgdimy-1-row][col])/maxfr*256)),255),0));
      }
    im.write(args[4]);
...
ComplexityBruteForce(std::ostream& outs) : Experiment(outs) {};
  ...
  virtual ui complexityMul(const mpz_class& number, const ui maxCompl)
  {
    ...
    else
    {
      ...
      std::vector<std::vector<ui> > pfindices(fi, std::vector<ui>(distpf, 0)); // holds the prime decomposition indices for each divisor (for easier recalculation of divindex)
	...
	if (totallb <= std::min(complexity-1, maxCompl))
	{
	  for (ui i = 0; i < fi; i++)
	  {
	    ...
	    quotient -= divindex[i];
	    if (quotient > 0 && cmp(divisors[quotient], m_fsize) < 0) // we're also done if we know the complexity of the remaining stuff
	    {
...
class FCalculator : public Experiment
{
  ...
  void calculate_finit_threaded()
  {
	...
	threads.push_back(std::thread(&FCalculator<PRINT>::finit_thread, this, i, std::ref(changed), start+bucketsize*i, start+bucketsize*(i+1)));
      threads.push_back(std::thread(&FCalculator<PRINT>::finit_thread, this, THREADS-1, std::ref(changed), start+bucketsize*(THREADS-1), finitsize));
